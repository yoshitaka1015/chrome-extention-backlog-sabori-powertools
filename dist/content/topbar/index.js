...
var SHADOW_HOST_ID = "backlog-manager-topbar-host";
var SPACER_ID = "backlog-manager-topbar-spacer";
var DATA_ORIGINAL_PADDING = "backlogManagerOriginalPaddingTop";
...
var MAX_SCAN_ELEMENTS = 1200;
var SECTION_LABEL_MARGIN_TOP = 1;
var SECTION_LABEL_MARGIN_BOTTOM = 1;
var BADGE_ROW_MARGIN_TOP = 0;
var BADGE_ROW_MARGIN_BOTTOM = 2;
var BAR_ACCENT_HEIGHT_PX = 5;
var LABEL_FONT_SIZE_PX = 11;
var LABEL_LINE_HEIGHT = 1.4;
var BADGE_HEIGHT_PX = 20;
var BADGE_BORDER_RADIUS_PX = 10;
var BADGE_FONT_SIZE_PX = 10;
var FOCUSED_BADGE_HEIGHT_PX = 24;
var FOCUSED_BADGE_RADIUS_PX = 12;
var FOCUSED_BADGE_FONT_SIZE_PX = BADGE_FONT_SIZE_PX + Math.round(2 * 96 / 72);
var ESTIMATED_BAR_HEIGHT = Math.round(
  SECTION_LABEL_MARGIN_TOP + LABEL_FONT_SIZE_PX * LABEL_LINE_HEIGHT + SECTION_LABEL_MARGIN_BOTTOM + BADGE_ROW_MARGIN_TOP + BADGE_HEIGHT_PX + BADGE_ROW_MARGIN_BOTTOM + BAR_ACCENT_HEIGHT_PX
);
var MINIMIZED_HEIGHT = 12;
var expandedBarHeight = ESTIMATED_BAR_HEIGHT;
var currentBarHeight = ESTIMATED_BAR_HEIGHT;
...
var DEFAULT_STATUS_COLOR = "#dc9926";
var FOCUS_DIM_BACKGROUND = "#d4d4d8";
var FOCUS_DIM_TEXT_COLOR = "#475569";
var CAKE_EMOJI = "\u{1F382}";
var CONFETTI_COLORS = ["#f97316", "#22d3ee", "#facc15", "#a855f7", "#38bdf8", "#fb7185"];
var CRACKER_CONFETTI_COLORS = ["#facc15", "#f97316", "#38bdf8", "#fb7185"];
var STORAGE_LEVEL_KEY = "backlogManagerLevel";
var SECTION_SEQUENCE = ["past", "today", "week", "noDue"];
var tooltipHideTimer;
var tooltipShowTimer;
var menuHideTimer;
var menuCurrentBadge = null;
var menuCurrentTask = null;
var documentClickHandlerAttached = false;
var activeStatusMenuRef = null;
var latestIssueBuckets = null;
var issuesRefreshTimer;
var ISSUES_REFRESH_INTERVAL_MS = 10 * 60 * 1e3;
var minimizedIndicator = null;
var minimizedIndicatorContext = null;
var pendingBarMeasurementFrame = 0;
...
async function loadMinimizedState() {
  try {
    const result = await storageGet([STORAGE_MINIMIZED_KEY]);
...
async function sendMessageWithRetry(payload, retries = 1, backoffMs = 300) {
  let attempt = 0;
  for (; ; ) {
    try {
...
async function loadStoredLevel() {
  try {
    const result = await storageGet([STORAGE_LEVEL_KEY]);
...
function parseFocusedTaskId(value) {
...
async function loadFocusedTaskId() {
  try {
    const result = await storageGet([STORAGE_FOCUSED_TASK_KEY]);
...
function persistFocusedTaskId(taskId) {
...
function setFocusedTask(context, taskId, options = {}) {
...
function toLocalDateKey(date) {
  ...
  const tzAdjusted = new Date(normalized.getTime() - normalized.getTimezoneOffset() * 6e4);
  return tzAdjusted.toISOString().split("T")[0];
}
function getTodayDateKey() {
  return toLocalDateKey(/* @__PURE__ */ new Date());
}
...
function isCompletionStatusName(name) {
  if (!name) {
    return false;
  }
...
function applyTopOffset(element, computedTop) {
...
function renderTopBar(shadowRoot, preferences) {
  ...
  const taskRegistry = /* @__PURE__ */ new WeakMap();
  const sections = /* @__PURE__ */ Object.create(null);
  const sectionContainers = /* @__PURE__ */ Object.create(null);
  ...
  togglePanelButton.addEventListener("click", async () => {
    ...
    void chrome.runtime.sendMessage({ type: "backlog:projects:details", force: true }).catch((error) => console.warn("Failed to prefetch project details:", error));
    try {
  ...
  if (!documentClickHandlerAttached) {
    document.addEventListener(
      ...
      (event) => {
        ...
        const menu = activeStatusMenuRef;
        if (menu && (path.includes(menu) || path.some((node) => node instanceof HTMLElement && node.classList?.contains("badge")))) {
          return;
        }
        if (menu) {
          hideStatusMenu(menu);
...
function createSection(label, key, context) {
...
function createActions() {
  ...
  minimizeButton.textContent = "\xD7";
...
\u3053\u306E\u8ABF\u5B50\u3067\u304C\u3093\u3070\u308A\u307E\u3057\u3087\u3046\uFF01\uFF01`;
  ...
  const randomizeConfetti = (piece, index) => {
  ...
  const sparkles = [];
  const randomizeSparkle = (sparkle, index) => {
    const left = Math.random() * 100;
  ...
  crackerBodyLeft.textContent = "\u{1F389}";
  const crackerBodyRight = document.createElement("div");
  crackerBodyRight.className = "celebration__cracker-body";
  crackerBodyRight.textContent = "\u{1F389}";
  const crackerPiecesLeft = [];
  const crackerPiecesRight = [];
...
var currentContextRef = null;
function createStatusMenuElement() {
  ...
  menu.addEventListener("click", (event) => {
    ...
    if (action === "status") {
      ...
      if (todayCountBeforeChange === 1 && isTaskDueToday(menuCurrentTask) && isCompletionStatusName(nextStatus)) {
      ...
      updateBadgeAppearance(menuCurrentBadge, menuCurrentTask, focusedTaskId === menuCurrentTask.id ? 40 : void 0);
      ...
      const desiredDate = /* @__PURE__ */ new Date();
...
function getProjectStatuses(context, projectId) {
...
function renderStatusMenuOptions(context, menu, task) {
...
async function ensureStatusMenuOptions(context, menu, task) {
...
async function applyStatusChange(context, task, statusId, statusName) {
  try {
    const response = await sendMessageWithRetry({
...
function updateCachedIssueStatus(issueId, statusName, statusId) {
...
async function applyDueDateChange(context, task, desiredDate) {
  ...
  const timezoneSafeDate = new Date(normalized.getTime() - normalized.getTimezoneOffset() * 6e4);
...
function updateCachedIssueDueDate(issueId, dueDate) {
  const buckets = latestIssueBuckets;
  if (!buckets) {
    return;
  }
...
function setSectionMessage(list, message, variant = "default") {
  list.innerHTML = "";
...
function setSectionsMessage(context, message, variant = "default") {
  SECTION_SEQUENCE.forEach((key) => {
...
function scheduleBarMeasurement(context) {
  if (isMinimized) {
    return;
  }
...
function renderIssueBuckets(context, buckets) {
...
function updateSectionVisibility(context, key, shouldShow, buckets = null) {
  const container = context.sectionContainers[key];
...
function renderSectionTasks(context, key, tasks) {
  const list = context.sections[key];
  if (!list) {
    return;
  }
...
function setBarMinimized(context, minimized) {
...
function updateHostHeight(height, minimized = false) {
...
function showMinimizedIndicator(context, visible) {
  const host = document.getElementById(SHADOW_HOST_ID);
  if (!visible) {
    if (host) {
...
function createBadgeElement(task, context) {
...
async function refreshIssues(context, options = {}) {
...
function hideStatusMenu(menu, immediate = false) {
  if (menuHideTimer) {
...
function updateBadgeAppearance(badge, task, overrideLength) {
...
function getBucketForKey(buckets, key) {
  switch (key) {
    case "past":
      return buckets.past;
    case "today":
...
function applyFocusModeAppearance(context) {
  const badgeEntries = [];
  SECTION_SEQUENCE.forEach((key) => {
    const list = context.sections[key];
    if (!list) {
...
function ensureFocusedTaskVisible(buckets) {
...
function updateLevelBadge(context) {
  if (!context) {
    return;
  }
...
function primeCelebrationEffects(elements) {
...
function showCelebrationPopup(context) {
  if (!context) {
    return;
  }
...
function hideCelebrationPopup(context) {
  if (!context) {
    return;
  }
  if (celebrationHideTimer) {
    window.clearTimeout(celebrationHideTimer);
    celebrationHideTimer = void 0;
  }
  delete context.celebration.overlay.dataset.open;
}
function handleTodayCompletionCelebration(context, todayCount) {
...
function scheduleTooltipForBadge(badge, task, tooltip) {
  if (tooltipShowTimer) {
    window.clearTimeout(tooltipShowTimer);
  }
  tooltipShowTimer = window.setTimeout(() => {
    tooltipShowTimer = void 0;
    showTooltipForBadge(badge, task, tooltip);
  }, 500);
}
function showTooltipForBadge(badge, task, tooltip) {
  if (tooltipHideTimer) {
    window.clearTimeout(tooltipHideTimer);
    tooltipHideTimer = void 0;
  }
  const title = tooltip.querySelector(".tooltip__title");
  const meta = tooltip.querySelector(".tooltip__meta");
...
function hideTooltip(tooltip, delay = 100) {
  if (tooltipShowTimer) {
    window.clearTimeout(tooltipShowTimer);
    tooltipShowTimer = void 0;
  }
...
async function toggleStatusMenuForBadge(badge, task, context) {
  hideTooltip(context.tooltip, 0);
...
async function showStatusMenuForBadge(badge, task, context) {
  if (menuHideTimer) {
    window.clearTimeout(menuHideTimer);
    menuHideTimer = void 0;
  }
  const { statusMenu } = context;
...
function updateStatusMenuSelection(menu, currentStatus) {
...
function positionFloatingElement(badge, floating, gap, constrainToViewport = false) {
  ...
  if (floating.classList.contains("tooltip")) {
    ...
    const arrowOffset = arrowCenter - left - 7;
...
function toggleSidePanel() {
  return sendMessageWithRetry({ type: "sidepanel:toggle" }, 2).then((response) => {
    if (!response?.ok) {
...
async function initTopBar() {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (BACKLOG_AUTH_KEY in changes) {
      const updated = changes[BACKLOG_AUTH_KEY]?.newValue;
